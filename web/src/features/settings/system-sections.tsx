...
export function CacheSection({
  ...
  const sizeLabel = isLoading ? '正在读取' : cache ? formatBytes(cache.sizeBytes) : '读取失败'
  const description = cache
    ? `Server 已缓存 ${formatNumber(cache.entryCount)} 个阅读图片文件`
    : '统计 Server 保存的阅读图片缓存'
    ...
    <SettingsSection icon={<DatabaseIcon className="size-4" />} title="缓存">
      <SettingRow title="服务端缓存" description={description}>
        <div className="flex items-center justify-between gap-3 sm:justify-end">
          ...
          <ConfirmDialog
            title="清除服务端缓存？"
            description="将删除 Server 已保存的阅读图片，后续阅读时会重新下载，不会删除下载任务记录。"
            confirmText="确认清除"
            variant="destructive"
            loading={isClearing}
            onConfirm={onClear}
